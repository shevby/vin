...
- [ ] 2.9 Trash — restoring entries from the trash directory (`pane.trash`, 2.7; deleting is permanent without one, as the user chose) to where they were, which needs their original paths kept beside them (as the freedesktop trash's `.trashinfo` files do), and emptying it. *(proposed)*
- [ ] 2.10 Long operations — background jobs with progress and cancel, so the UI stays responsive (2.7's operations run in the command, with no progress); the conflict prompt (overwrite / merge / skip / keep both / for all) is 2.7's.
