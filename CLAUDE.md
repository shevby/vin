...
- Backend:
  ...
  - Windows (`src/windows.js`) — the backend owns the stack and the focus, mirrored in `core`'s `windows` state (bottom to top: `id`, `path`, `kind`, `focus`). `await vin.openWindow('main')` opens a lasting window, there until its handler is disposed; `await this.openWindow(new Confirm(…))` adds a transient one to the opener as a sub-handler, on top with the focus, and resolves with its result — `this.close(result)` inside it, or `null` for Escape (`core.closeWindow`) or disposal. Closing disposes it and the windows it opened. `this.focus()` takes the focus within a handler's window (called before the window opens — on a sub-handler, in the window's `onInit()` — it picks the focus the window opens with); each window keeps its own for when it's uncovered.
    - Main window (`src/handlers/main/`) — `index.js` registers it and `vin.start({ window: 'main' })` opens it. Two panes, `main.left` and `main.right` (kind `pane`), each showing a directory by URI; the active one (`state.active`) has the focus, so pane commands act on it, and changes only through `main`'s commands. Single-pane mode (`state.singlePane`) shows just the active one. A pane lists its directory as `entries` in state — names and types first, `stat` details filled in afterwards in batches, each one update (a per-entry edit would copy the whole array in the UI per message) — and the UI renders only the rows that fit (`ui/tui/main/pane/listing.jsx`), reporting how many (`setPageSize()`) for the page keys. Moving to another directory keeps the old listing until the new one is read, so a failure leaves the pane where it was; each pane keeps its own history and the cursor's entry per directory it left, in private fields for now (2.22 will save them). A file opens with the OS default app (`src/open.js`; launched apart from vin, never through a shell, and not awaited, so a failure is reported when it comes).
