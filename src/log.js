...
/**
 * `vin.log` in the project root (git-ignored), where errors go when `VIN_LOG` isn't set. It's found through
 * `package.json` rather than `__dirname`, which the TUI bundle (ESM) doesn't have; this file and the bundle
 * both sit one level below the root.
 */
const DEFAULT_LOG_FILE = path.join(path.dirname(require.resolve('../package.json')), 'vin.log');
 ...
 * @param {boolean} [options.optional] The file is created by the first line written, and failing to write
 *   is ignored — for the default log, which must never stop vin. Otherwise it's opened now, so a bad path
 *   fails at startup, not at the first log call.
 * @returns {Logger}
...
function createLogger(file, { level = 'debug', optional = false } = {}) {
  ...
  const writer = (name) => {
    ...
    return (/** @type {unknown[]} */ ...args) => {
      try {
        fs.appendFileSync(target, `${new Date().toISOString()} ${name.toUpperCase()} ${util.format(...args)}\n`);
      } catch (error) {
        if (!optional) {
          throw error;
        }
 ...
 * empty, and when it's unset, errors only to `vin.log` in the project root — so an unexpected error is
 * never lost.
