 ...
 * submitted again.
 * @extends {Handler<{ title: string | null, message: string | null, error: string | null }>}
 */
class Prompt extends Handler {
   ...
   */
  constructor({ message, title, value, cursor, secret, validate, name = 'prompt' } = {}) {
    super(name);
    ...
    if (validate !== undefined && typeof validate !== 'function') {
      throw new TypeError(`Prompt "${name}": validate must be a function`);
    }
  ...
  onInit() {
    this.update({ ...this.#initial, error: null });
    this.input.focus();
  }
